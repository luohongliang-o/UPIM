...
// 用户日志
#ifndef USER_LOG
#define USER_LOG(pszFormat, ...)	WriteUserLog(pszFormat, __VA_ARGS__)
#endif
...
// 比较2个UPID是否相等
inline BOOL EqualUPID(LPCUSERID pSrcUPID, LPCUSERID pDesUPID){return strcmp(pSrcUPID, pDesUPID)==0;}
inline BOOL EqualUPID(LPVUPID pSrcUPID, LPCUSERID pDesUPID){return strcmp(pSrcUPID->m_szIMID, pDesUPID)==0;}
inline BOOL EqualUPID(VUPID& SrcUPID, LPCUSERID pDesUPID){return strcmp(SrcUPID.m_szIMID, pDesUPID)==0;}
inline BOOL EqualUPID(LPVUPID pSrcUPID, LPVUPID pDesUPID){return strcmp(pSrcUPID->m_szIMID, pDesUPID->m_szIMID)==0;}
inline BOOL EqualUPID(VUPID& SrcUPID, VUPID& DesUPID){return strcmp(SrcUPID.m_szIMID, DesUPID.m_szIMID)==0;}
inline BOOL EqualUPID(LPCUSERID pSrcUPID, LPVUPID pDesUPID){return strcmp(pSrcUPID, pDesUPID->m_szIMID)==0;}
inline BOOL EqualUPID(LPCUSERID pSrcUPID, VUPID& DesUPID){return strcmp(pSrcUPID, DesUPID.m_szIMID)==0;}

// UPID赋值
inline void CopyUPID(LPVUPID pDesUPID, LPCUSERID pSrcUPID){COPYSTRARRAY(pDesUPID->m_szIMID, pSrcUPID);}
inline void CopyUPID(VUPID& DesUPID, LPCUSERID pSrcUPID){COPYSTRARRAY(DesUPID.m_szIMID, pSrcUPID);}
inline void CopyUPID(LPVUPID pDesUPID, LPVUPID pSrcUPID){COPYSTRARRAY(pDesUPID->m_szIMID, pSrcUPID->m_szIMID); pDesUPID->m_nUserType=pSrcUPID->m_nUserType;}
inline void CopyUPID(VUPID& DesUPID, VUPID& SrcUPID){COPYSTRARRAY(DesUPID.m_szIMID, SrcUPID.m_szIMID);DesUPID.m_nUserType=SrcUPID.m_nUserType;}

inline BOOL EqualRECT(RECT a, RECT b){return (a.bottom == b.bottom)&&(a.left == b.left)&&(a.right == b.right)&&(a.top == b.top);}
