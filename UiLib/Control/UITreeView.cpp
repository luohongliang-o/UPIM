...
namespace DuiLib
{
	CTreeNodeUI::CTreeNodeUI( CTreeNodeUI* _ParentNode /*= NULL*/ )
	{
		...
		pTreeView		= NULL;
		m_bIsVisable	= false;  //应该为false,原来为true
		m_bIsCheckBox	= false;
...
//			pItem->SetVisibleTag(pFolder->GetCheck());		//这两行代码无用，pFolder->Selected(!pFolder->IsSelected()); 会触发OnFolderChanged函数，OnFolderChanged函数会执行这两行代码
//			SetItemExpand(pFolder->GetCheck(),pItem);
			return true;
		}
		return false;
