...
BOOL CTxtWinHost::Init(CRichEditUI *re, const CREATESTRUCT *pcs)
{
	...
	HMODULE hmod = LoadLibrary(_T("msftedit_win7.dll"));
	if (hmod == NULL )
		hmod = LoadLibrary(_T("IM/msftedit_win7.dll"));
...
CRichEditUI::CRichEditUI() : m_pTwh(NULL), m_bVScrollBarFixing(false), m_bWantTab(true), m_bWantReturn(true), 
    ...
    m_iLimitText(cInitTextMax), m_lTwhStyle(ES_MULTILINE), m_bInited(false), m_bReg(false)
{
}
