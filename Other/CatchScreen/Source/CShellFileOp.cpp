...
#include "shlobj.h"
#include "objbase.h"
#include "..\include\CShellFileOp.h"
...
int    nAPIRet;
    ...
    if ( ! m_bFlagsSet )
        {
        TRACE0(_T("Go() aborting because SetOperationFlags() was not called first.\n"));
        goto bailout;
        }
    ...
    if ( ! ( m_rFOS.wFunc == FO_COPY  ||  m_rFOS.wFunc == FO_DELETE  ||
        ...
        {
        TRACE0(_T("Go() aborting because the operation type was invalid.\n"));
        goto bailout;
        }
    ...
    if ( m_lcstrSourceFiles.IsEmpty() ) 
        {
        TRACE0(_T("Go() aborting because the source file list is empty.\n"));
        goto bailout;
        }
    ...
    if ( m_rFOS.wFunc != FO_DELETE  &&  m_lcstrDestFiles.IsEmpty() )
        {
        TRACE0(_T("Go() aborting because the destination file list is empty.\n"));
        goto bailout;
        }
    ...
    if ( m_rFOS.wFunc != FO_DELETE )
        ...
        if ( m_lcstrDestFiles.GetCount() != 1  &&
            ...
            {
            TRACE0(_T("Go() aborting because the destination file list has the wrong number of strings.\n"));
            goto bailout;
            }
        }
    ...
    try
        {
        szzSourceFiles = new TCHAR [ dwSourceBufferSize ];

        if ( m_rFOS.wFunc != FO_DELETE )
            {
            szzDestFiles = new TCHAR [ dwDestBufferSize ];
            }
        }
    catch ( CMemoryException )
        {
        TRACE0(_T("Memory exception in CShellFileOp::Go()!\n"));
        throw;
        }
