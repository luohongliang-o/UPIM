...
void RichEdit_ReplaceSel(HWND hWnd, LPCTSTR lpszNewText, BOOL bCanUndo/* = FALSE*/)
{
	...
	::SendMessage(hWnd, EM_REPLACESEL, (WPARAM)bCanUndo, (LPARAM)lpText);
	delete[] lpText;
#endif
}
						 ...
						 int nFaceIndex, COLORREF clrBg, BOOL bAutoScale, int nReservedWidth)
{
	...
	bstrFileName = ::SysAllocString(CT2OLE(lpszFileName));
...
void RichEdit_GetText(HWND hWnd, tstring& strText)
{
	...
	for (LONG i = 0; i < (int)strOrgText.size(); i++)
	{
		...
		if (SUCCEEDED(hr))
		{
			...
			{
				//////////////////////////////////////////////////////////////////////////
				// string的截取，汉字算成2个字符，导致截取出现乱码
#if defined(UNICODE)
				strTemp = strOrgText.substr(nPos, reobject.cp-nPos);
				Replace(strTemp, _T("/"), _T("//"));
				strText += strTemp;
#else
				wstring strRes = g_CharConv.StringToWstring(strOrgText);
				wstring strTmp = strRes.substr(nPos, reobject.cp-nPos);
				string strtmp2 = g_CharConv.WstringToString(strTmp);
				Replace(strtmp2, _T("/"), _T("//"));
				strText += strtmp2;
#endif
			}
			nPos = reobject.cp + 1;
...
#else
		wstring strRes = g_CharConv.StringToWstring(strOrgText);
		wstring strTmp = strRes.substr(nPos);
		string strtmp2 = g_CharConv.WstringToString(strTmp);
		Replace(strtmp2, _T("/"), _T("//"));
		strText += strtmp2;
#endif
...
int RichEdit_GetTextRange(ITextServices * pTextServices, CHARRANGE * lpchrg, CDuiString& strText)
{
	...
	::ZeroMemory(lpText, (nLen + 1) * sizeof(WCHAR));
...
void RichEdit_ReplaceSel(ITextServices * pTextServices, LPCTSTR lpszNewText, BOOL bCanUndo/* = FALSE*/)
{
	...
	::ZeroMemory(lpText, (iLen + 1) * sizeof(WCHAR));
	::MultiByteToWideChar(CP_ACP, 0, lpszNewText, -1, (LPWSTR)lpText, iLen) ;
	pTextServices->TxSendMessage(EM_REPLACESEL, (WPARAM)bCanUndo, (LPARAM)lpText, NULL);
	delete[] lpText;
#endif
}
...
BOOL RichEdit_InsertFace(ITextServices *pTextServices, ITextHost *pTextHost,	
						 ...
						 COLORREF clrBg, BOOL bAutoScale, int nReservedWidth, IMAGE_INFO *pII /* = NULL */)
{
	BSTR bstrFileName = NULL;
	IRichEditOle * pRichEditOle = NULL;
	IOleClientSite *pOleClientSite = NULL;
...
void RichEdit_GetText(ITextServices * pTextServices, tstring& strText)
{
	...
	CDuiString strText_test = "";
	RichEdit_GetTextRange(pTextServices, &chrg, strText_test);
	strOrgText = strText_test.GetData();
	for (LONG i = 0; i < (int)strOrgText.size(); i++)
	{
		memset(&reobject, 0, sizeof(REOBJECT));
		reobject.cbStruct = sizeof(REOBJECT);
			...
			if (CLSID_ImageOle == reobject.clsid)
			{
				...
				if (SUCCEEDED(hr))
				{
					...
					else
					{
						...
						BSTR bstrFileName = NULL;
						pImageOle->GetFileName(&bstrFileName);
						// 把得图像拷贝到用户的image文件夹下，并命名为guid的文件进行发送
						CString strOldFileInfo(bstrFileName);
						CString strSuffix = strOldFileInfo.Right(strOldFileInfo.GetLength()-strOldFileInfo.ReverseFind('.')-1);
						CString newFileName = "",newFilePath = "";
						newFileName.Format("%s.%s", MakeGuid(), strSuffix);
						newFilePath.Format("%s\\%s", g_config.szUserImage, newFileName);
						//BOOL bCopyFile = CopyFile(strOldFileInfo, newFilePath, false); 

						// 对于源图像进行处理，假如是尺寸太大的图像，压缩处理之后再发送
						HandleImage(strOldFileInfo, newFilePath);

						BSTR bstrNowFileName = newFileName.AllocSysString();
						//////////////////////////////////////////////////////////////////////////
						//strText += COLE2T(bstrFileName);
						strText += COLE2T(bstrNowFileName);
						::SysFreeString(bstrFileName);
						::SysFreeString(bstrNowFileName);
						strText += _T("\"]");
					}
					pImageOle->Release();
				}
