...
CMenuElementUI* CMenuWnd::GetMenuItem(LPCTSTR lpszName)
{
	if (lpszName != NULL)
	{
		return static_cast<CMenuElementUI*>(m_pm.FindControl(lpszName));
	}
	return NULL;
}
...
{
	CMenuElementUI* pMenuElement = static_cast<CMenuElementUI*>(m_pm.FindControl(lpszName));;
	if (pMenuElement != NULL)
		pMenuElement->SetCheckItem(bCheck==TRUE);
}
...
void CMenuWnd::SetIconAndSize(LPCTSTR lpszName, LPCTSTR strIcon, LONG cx, LONG cy)
{
	CMenuElementUI* pMenuElement = static_cast<CMenuElementUI*>(m_pm.FindControl(lpszName));;
	if (pMenuElement != NULL)
	{
		pMenuElement->SetIcon(strIcon);
		pMenuElement->SetIconSize(cx, cy);
	}
}
...
LRESULT CMenuWnd::OnCreate(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
	if( m_pOwner != NULL) {
		...
		RECT rcPreWindow = {0};
		ObserverImpl_::Iterator_ iterator(CMenuWnd::GetGlobalContextMenuObserver());
		ReceiverImplBase_* pReceiver = iterator.next();
