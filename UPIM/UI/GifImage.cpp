...
BOOL CGifImage::LoadFromFile(LPCTSTR pszFileName)
{
	...
	WStrFileName = (wchar_t*)malloc(len*sizeof(wchar_t));
	setlocale(LC_ALL, "");
	mbstowcs_s(&converted, WStrFileName, len, pszFileName, _TRUNCATE);
	//////////////////////////////////////////////////////////////////////////
	USES_CONVERSION;
	#undef new
	m_pImage = new Gdiplus::Image(WStrFileName);
	if (NULL == m_pImage)
		return FALSE;
...
BOOL CGifImage::SaveAsFile(LPCTSTR pszFileName)
{
	...
	size_t converted = 0;
	wchar_t *WStrExtension;
	WStrExtension = (wchar_t*)malloc(len*sizeof(wchar_t));
	mbstowcs_s(&converted, WStrExtension, len, lpExtension, _TRUNCATE);
	//////////////////////////////////////////////////////////////////////////

	CLSID clsid = GetEncoderClsidByExtension(WStrExtension);
	if (CLSID_NULL == clsid)
		return FALSE;
	free(WStrExtension);
	//////////////////////////////////////////////////////////////////////////
	len = strlen(pszFileName) + 1;
	wchar_t *WStrFileName;
	WStrFileName = (wchar_t*)malloc(len*sizeof(wchar_t));
	mbstowcs_s(&converted, WStrFileName, len, pszFileName, _TRUNCATE);
	//////////////////////////////////////////////////////////////////////////

	Gdiplus::Status status = m_pImage->Save(WStrFileName, &clsid, NULL);
	free(WStrFileName);
